...
function iterateStack (path, routeLayer, layer, keys, cb) {
  cb(path, routeLayer, layer, keys)

  if (layer.name === 'router') {
    layer.handle.stack.forEach(l => {
      path = path || ''
      iterateStack(path + split(layer.regexp, layer.keys).join('/'), layer, l, [...keys, ...layer.keys], cb)
    })
  }
  if (!layer.route) {
    return
  }
  layer.route.stack.forEach((l) => iterateStack(path + layer.route.path, layer, l, [...keys, ...layer.keys], cb))
}
